...
import esm_bo
from esm_bo.modules import ContactPredictionHead, ESM1bLayerNorm, RobertaLMHead, TransformerLayer, TransformerAdapterLayer
from esm_bo.adapter import ResMLP
...
class ESM2(nn.Module):
    def __init__(
        ...
        alphabet: Union[esm_bo.data.Alphabet, str] = "ESM-1b",
        token_dropout: bool = True,
        num_end_adapter_layers: List[int]=None,
        ...
        if not isinstance(alphabet, esm_bo.data.Alphabet):
            alphabet = esm_bo.data.Alphabet.from_architecture(alphabet)
        ...
        if num_end_adapter_layers is None:
    ...
    def _init_submodules_adapter(self):
        ...
        for adapter_idx, value in enumerate(self.num_end_adapter_layers):
    ...
    def get_embed_tokens(self, tokens):
        """
        Get the embedding tokens for the given input tokens.

        Args:
            tokens (list): A list of input tokens.

        Returns:
            list: A list of embedding tokens.
        """
        return self.embed_tokens(tokens)

    def forward(self, tokens, embed_token_x=None, repr_layers=[], need_head_weights=False, return_contacts=False):
        if return_contacts:
            need_head_weights = True

        assert tokens.ndim == 2
        
        if embed_token_x is not None:
            padding_mask = tokens.eq(self.padding_idx)  # B, T
            x = self.embed_scale * embed_token_x
        else:
            if (self.prefix_module is not None and 
                self.prefix_module.prompt_layer_index == 0):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if (self.prefix_module is not None and 
                self.prefix_module.prompt_layer_index == layer_idx and
                layer_idx != 0):
